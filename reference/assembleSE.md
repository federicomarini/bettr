...
Assemble all bettr input into a `SummarizedExperiment` object. This has
the advantage of keeping all data together in a single object, and can
be used as input to `bettr` or `bettrGetReady`, instead of providing the
...
A SummarizedExperiment object with rows corresponding to methods and
columns corresponding to metrics.
