...
makeBarPolarPlot <- function(bettrList = NULL,
                             ...
                             showComposition = FALSE, scaleFactorPolars = 1.0) {
    ...
    rplots <- lapply(methods, function(m) {
        ggplot2::ggplot(dplyr::filter(plotdata, .data[[idCol]] == m),
                        ggplot2::aes(x = .data[[metricCol]],
                                     y = .data[[valueCol]],
                                     fill = .data[[metricCol]])) +
            ggplot2::geom_col(width = 1.0, color = "white") +
            ggplot2::ylim(min(0.0, min(plotdata[[valueCol]], na.rm = TRUE)),
            ...
            ggplot2::theme(
                ...
                panel.spacing = grid::unit(0.0, "cm")
            ) +
            ggplot2::labs(x = "", y = "")
    })
    ...
    legnd <- cowplot::get_legend(rplots[[1L]])
    ...
    levs <- dplyr::pull(scoredata, .data[[idCol]])
    rx <- length(levs)
    ry <- max(0.0, max(scoredata[[scoreCol]])) -
        min(0.0, min(scoredata[[scoreCol]]))
    sx <- 2.5
    sy <- ry / rx * sx
    ...
    if (showComposition) {
        ...
        bplot <- ggplot2::ggplot(dplyr::mutate(plotdata, "{idCol}" :=
                                                   factor(.data[[idCol]],
                                                          levels = levs)),
        ...
        bplot <- ggplot2::ggplot(dplyr::mutate(scoredata, "{idCol}" :=
                                                   factor(.data[[idCol]],
                                                          levels = levs)),
                                 ggplot2::aes(x = .data[[idCol]],
                                              y = .data[[scoreCol]])) +
    ...
    bplot <- bplot +
        ...
        ggplot2::theme(
            axis.text.x = ggplot2::element_text(
                angle = 90.0, hjust = 1.0, vjust = 0.5, size = labelSize
            ),
    ...
    cowplot::plot_grid(bplot, legnd, rel_widths = c(1.0, 0.2), nrow = 1L)
}
