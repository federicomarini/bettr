...
.checkArgs_makeHeatmap <- function(
    ...
    .assertScalar(x = rownamewidth_cm, type = "numeric", rngIncl = c(0.0, Inf))
    .assertScalar(x = colnameheight_cm, type = "numeric", rngIncl = c(0.0, Inf))
}
...
makeHeatmap <- function(bettrList = NULL,
                        ...
                        plotType = "Heatmap",
                        rownamewidth_cm = 6.0, colnameheight_cm = 6.0) {
    ...
    if (metricGroupCol %in% colnames(colAnnot)) {
        colAnnot <- dplyr::arrange(colAnnot, .data[[metricGroupCol]])
