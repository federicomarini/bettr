test_that("transformVariable works", {
    ...
    expect_identical(.transformNumericVariable(x = x, flip = FALSE, offset = 0,
    ...
    expect_identical(t1[5], 0)
    expect_identical(t1[2], 1)

    t1 <- .transformNumericVariable(x = x, flip = FALSE, offset = 0,
                                    transf = .getTransf("[-1,1]"),
                                    bincuts = NULL)
    expect_identical(t1, (2 * x - (min(x, na.rm = TRUE) + max(x, na.rm = TRUE))) /
                         (max(x, na.rm = TRUE) - min(x, na.rm = TRUE)))
    expect_identical(min(t1, na.rm = TRUE), -1.0)
    expect_identical(max(t1, na.rm = TRUE), 1.0)
    expect_length(t1, length(x))
    expect_identical(t1[5], -1)
    expect_identical(t1[2], 1)

    t1 <- .transformNumericVariable(x = x, flip = FALSE, offset = 0,
                                    transf = .getTransf("Rank"),
                                    bincuts = NULL)
    expect_identical(t1, c(3, 5, 2, 4, 1, NA))

    t1 <- .transformNumericVariable(x = x, flip = FALSE, offset = 0,
                                    transf = .getTransf("Rank+[0,1]"),
                                    bincuts = NULL)
    expect_identical(t1, c(0.5, 1, 0.25, 0.75, 0, NA))
    ...
    expect_identical(.transformNumericVariable(x = x, flip = TRUE, offset = 0,
                                               transf = .getTransf("None"),
                                               bincuts = NULL),
                     -x)
    ...
    t1 <- .transformNumericVariable(x = x, flip = TRUE, offset = 0,
                                    transf = .getTransf("Rank"),
                                    bincuts = NULL)
    expect_identical(t1, c(3, 1, 4, 2, 5, NA))

    t1 <- .transformNumericVariable(x = x, flip = TRUE, offset = 0,
                                    transf = .getTransf("Rank+[0,1]"),
                                    bincuts = NULL)
    expect_identical(t1, c(0.5, 0, 0.75, 0.25, 1, NA))
    ...
    expect_identical(.transformNumericVariable(x = x, flip = FALSE, offset = 1,
                                               transf = .getTransf("None"),
                                               bincuts = NULL),
                     x + 1)

    expect_identical(.transformNumericVariable(x = x, flip = TRUE, offset = 1,
                                               transf = .getTransf("z-score"),
                                               bincuts = NULL),
    ...
    t1 <- .transformNumericVariable(x = x, flip = FALSE, offset = 0,
                                    transf = .getTransf("[0,1]"),
                                    bincuts = c(0, 2))
    expect_identical(t1, c(1, 1, 1, 1, 1, NA))

    t1 <- .transformNumericVariable(x = x, flip = FALSE, offset = 0,
                                    transf = .getTransf("[-1,1]"),
                                    bincuts = c(0, 2))
    expect_identical(t1, c(2, 2, 2, 2, 1, NA))
